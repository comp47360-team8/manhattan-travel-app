...
def assign_days(pois_list: list[POI], pois: list[POIProfile], trip_days: list[int]):
    """
    Assign POIs to trip days while respecting opening day constraints.

    POIs available across all trip days are distributed evenly across days.
    POIs with restricted availability are locked to a compatible day with
    the lowest current number of assigned POIs.

    Args:
        pois_list: Full POI database objects used for metadata lookup.
        pois: Processed POI profiles containing availability information.
        trip_days: Days of the week included in the user's trip.

    Returns:
        A dictionary mapping weeks and weekdays to assigned POI profiles.

    Raises:
        POINotOpenDuringTrip: If a POI is unavailable for all trip days.
    """
    ...
    # find pois not opened on all days
    # if poi not open on every day of trip, stop engine
    pois_to_lock = []
    for poi in pois:
...
def assign_slots(pois: list[POIProfile], calendar: dict[int, dict[int, list]], trip_days: list, db: Session):
    """
    Assign POIs to time slots based on predicted busyness.

    Each POI is evaluated against available morning, afternoon, and evening
    slots. The lowest-busyness valid slot is selected where capacity allows.
    Overflow cases are resolved using combined busyness and geographic costs.

    Args:
        pois: POI profiles containing availability information.
        calendar: Day-level POI assignments generated by assign_days.
        trip_days: Days included in the itinerary.
        db: Database session.

    Returns:
        A tuple containing:
            - Optimized itinerary grouped by week/day/time slot.
            - Optional warning message if slot capacity is exceeded.
    """
...
def overflow(poi: POIProfile, matrix: dict, slot: dict, week: int, itinerary: dict, db: Session, pois: list[POIProfile]):
    """
    Resolve scheduling conflicts when a time slot exceeds capacity.

    Attempts to move the least suitable POI to a later slot by comparing
    busyness impact and geographic cost. This prevents itinerary generation
    failures while preserving lower-cost schedules.

    Args:
        poi: POI that could not be placed in its preferred slot.
        matrix: Busyness forecast matrix for all POIs.
        slot: Current overloaded time slot.
        week: Trip week index.
        itinerary: Current itinerary structure.
        db: Database session.
        pois: All POI profiles.
    """
...
def assign_to_next_slot(to_move: POIProfile, slot: dict, week: int, itinerary: dict, matrix, db, pois):
    """
    Move a POI into the next available time slot.

    If the next slot is also full, recursively applies overflow handling.
    """
